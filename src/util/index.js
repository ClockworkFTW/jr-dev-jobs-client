...
export const reduceJobs = ({ jobs, filters, sort, location }) => {
  let reducedJobs;

  // Filter jobs
  const { search, company, technologies, remote, applied } = filters;
  reducedJobs = jobs.data.filter((job) => {
  ...
  // Sort jobs
  const { distance, rating, salary } = sort;
  reducedJobs = reducedJobs.sort((a, b) => {
    ...
    if (salary) {
      if (salary === "asc") {
        return a.salary[0].Total.slice(1, -1) - b.salary[0].Total.slice(1, -1);
      }
      if (salary === "dsc") {
        return b.salary[0].Total.slice(1, -1) - a.salary[0].Total.slice(1, -1);
      }
    }
  });
